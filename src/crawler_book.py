...
def get_rating(soup):
    ...
    if rating == None:
        return None
    rating_dict = {}
    rating_dict['评分'] = rating.find('strong',
                                    class_='ll rating_num').get_text()
    rating_dict['评分人数'] = rating.find(
        'a', class_='rating_people').find('span').get_text()
    # for i in rating.find('div', class_ = 'ratings-on-weight').find_all('div', class_ = 'item'):
    for i in range(1, 6):
        rating_dict[str(i) + '星'] = rating.find(
            'span', class_='rating_per').get_text()
    return rating_dict
...
def get_content(url):
    ...
    if soup.find('span', class_="all hidden") != None:
        intro = "".join(
            soup.find('span', class_="all hidden").get_text().split())
    else:
        intro = "".join(soup.find('div', class_='intro').get_text().split())
...
if __name__ == '__main__':
    ...
    id_path = path + "/../data/Book_id.csv"
    info_path = path + "/../data/Book_info.json"
    error_path = path + "/../data/Book_error_id.txt"
    fread = open(id_path, "r")

    #check if file book_info.json exisit
    try:
