...
def get_rating(soup):
    ...
    star = 5
    for i in rating.find('div',
                         class_='ratings-on-weight').find_all('div',
                                                              class_='item'):
        rating_dict[str(star) + '星'] = i.find('span',
                                              class_='rating_per').get_text()
        star -= 1
    return rating_dict
...
def get_content(url):
    ...
    if soup.find('span', class_="all hidden") != None:
        ...
        intro = "".join(
            soup.find('span', property='v:summary').get_text().split())
